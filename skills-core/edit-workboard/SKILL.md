...
2. If the request is task selection, dependency triage, or next-task identification, hand off to `{{CMD_PREFIX}}query-workboard`.
3. If the request is executing a task lifecycle (`todo -> in_progress -> done`), hand off to `{{CMD_PREFIX}}start-task`.
...
`status` is write-protected in this skill. The only legal status writes are the atomic `set-blocked` and `unblock` operations below. Execution transitions belong to `{{CMD_PREFIX}}start-task`.
...
- Never edit `status` via `edit-task`; use `set-blocked`, `unblock`, or `{{CMD_PREFIX}}start-task`.
- Never rename an `id`.
